...
void GLApplication::sectionCircle() {
    ...
    _section.clear();
    for(int i = 0; i<=nBCirclePoint; i++){
        float teta = 2 * 3.1416 * i/nBCirclePoint;
        _section.push_back(Vector2(cosf(teta),sinf(teta)));
    }
...
void GLApplication::pathCircle() {
    int nBCirclePoint = 30;
    _path.clear();
    for(int i = 0; i<=nBCirclePoint+1; i++){
        float teta = 2 * M_PI * i/nBCirclePoint;
        _path.push_back(Vector3(cosf(teta),0 ,sinf(teta)));
    }
}
...
Vector3 GLApplication::pointSpline(double tNormalized) {
    ...
    Vector3 p0 = _path[i], p1 = _path[i+1];
    Vector3 t0 = tangentPathLine(i), t1 =tangentPathLine(i+1);
...
Vector3 GLApplication::tangentPathSpline(double tNormalized) {
    ...
    if(tNormalized == 0)
        result = pointSpline(0.01) - pointSpline(0);

    else if(tNormalized == 1)
        result = pointSpline(1) - pointSpline(0.99);

    else
        result = pointSpline(tNormalized + 0.01) - pointSpline(tNormalized - 0.01);
...
void GLApplication::extrudeLine() {
    ...
    //            Vector2 tmp = _section[j];
    //            _extrusion.push_back(Vector3(tmp.x(), tmp.y(), _path[i].z()));
    //        }
    //    }
...
void GLApplication::extrudeSpline() {
    ...
    for(double i = 0; i<1; i+=0.01){
        for(int j = 0; j< _section.size(); j++){
            _extrusion.push_back(pointSpline(i) + rotatePlane(Vector3(_section[j], 0), tangentPathSpline(i)));
        }
    }
